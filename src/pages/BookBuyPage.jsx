...
import {
  ...
  ArrowLeft, ShieldCheck, LogIn, UserPlus, ExternalLink,
} from 'lucide-react'
...
export default function BookBuyPage() {
  ...
  const { ownedBookIds, refresh: refreshPurchases } = usePurchases()
  ...
  // If user already owns this book, redirect to my-books
  useEffect(() => {
    if (user && book && ownedBookIds.has(book.id)) {
      navigate('/sach-cua-toi', { replace: true })
    }
  }, [user, book, ownedBookIds, navigate])
        ...
        <ScrollReveal>
          <div className="max-w-md mx-auto bg-white rounded-3xl shadow-section p-8 text-center">
            ...
            <p className="text-gray-500 mb-6 text-sm">
              Vui lòng đăng nhập hoặc tạo tài khoản để tiếp tục đặt mua sách.
            </p>

            {/* Book preview */}
            <div className="bg-gray-50 rounded-2xl p-4 mb-6 text-left flex items-center gap-3">
              {book.image && <img src={book.image} alt="" className="w-12 h-12 rounded-lg object-contain bg-white shrink-0" />}
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-gray-800 line-clamp-1">{book.name}</p>
                <p className="text-sm font-bold text-brand-700 mt-0.5">{formatPrice(book.price)}đ</p>
              </div>
            </div>

            <div className="space-y-3">
              <Link to={`/login?redirect=/sach/${id}/mua`}
              ...
              <Link to={`/register?redirect=/sach/${id}/mua`}
        ...
        <ScrollReveal>
          <div className="max-w-md mx-auto bg-white rounded-3xl shadow-section p-8 lg:p-12 text-center">
            ...
            <p className="text-gray-400 mb-6 text-sm">
              Sách đã được thêm vào tài khoản của bạn. Bạn có thể xem lại bất cứ lúc nào.
            </p>
            <Link
              to="/sach-cua-toi"
              className="w-full h-12 rounded-2xl font-semibold text-sm bg-emerald-600 text-white
                         hover:bg-emerald-700 flex items-center justify-center gap-2 transition-all duration-200 mb-3"
            >
        ...
        <ScrollReveal>
          <div className="max-w-lg mx-auto bg-white rounded-3xl shadow-section p-8 lg:p-10">
            ...
            <div className="bg-brand-50 rounded-2xl p-4 mb-6 flex items-center gap-3">
              {book.image && (
                <img src={book.image} alt="" className="w-12 h-12 rounded-xl object-contain bg-white shrink-0" />
              )}
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-brand-900 line-clamp-1">{book.name}</p>
